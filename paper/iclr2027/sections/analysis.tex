...
Table~\ref{tab:failure-modes} categorizes missed events and false alarms. The most common failures are not cases where no cheap signal exists; they are cases where cheap evidence is ambiguous or arrives at the wrong temporal granularity. This suggests that future work should learn when to increase local frame rate, query crops, or ask a more specific visual question.

\begin{table}[t]
\centering
\small
\caption{Failure modes for \method{} under the main budget. Shares are normalized over inspected failures.}
\label{tab:failure-modes}
\input{tables/failure_modes}
\end{table}
