...
The difficulty is not simply that the stream count is large. The correct observation can be valuable for different reasons at different times: a high-motion stream may be a real incident, a static stream may contain an abandoned object, and a previously queried stream may deserve reinspection only if new evidence appears. A useful benchmark must therefore expose the full allocation problem: sparse event intervals, hard background distractors, asynchronous stream histories, explicit VLM-call limits, and metrics that penalize late or noisy alerts. A useful method must likewise decide when cheap evidence is enough, when uncertainty itself is valuable, and when a stream has already consumed enough attention.
...
\begin{enumerate}
    ...
    \item We report a complete evaluation protocol: dataset tracks, budget sweeps, stream-count scaling, latency CDFs, false-alarm accounting, per-dataset breakdowns, ablations, and failure modes.
